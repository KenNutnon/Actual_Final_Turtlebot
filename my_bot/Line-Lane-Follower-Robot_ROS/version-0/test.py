...
class Follower:
  ...
  def image_callback(self, msg):
    ...
    if M['m00'] > 0:
      ...
      # BEGIN CONTROL
      err = cx - 4*w/8
      self.twist.linear.x = 0.9*0
      dt = rospy.get_time() - ptime
      self.twist.angular.z = ((-float(err) / 100)*2.5 + ((err - perr)/(rospy.get_time() - ptime))*1/50/100)*0 #+ (serr*dt)*1/20/100 #1 is best, starting 3 unstable
      serr = err + serr
      perr = err
