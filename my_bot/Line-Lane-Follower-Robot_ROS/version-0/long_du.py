...
class Follower:
  ...
  def image_callback(self, msg):
    ...
    (thresh, rgb_yw) = cv2.threshold(rgb_yw, 100, 255, cv2.THRESH_BINARY)
    ...
    if M['m00'] > 0:
      ...
      self.twist.angular.z = 0#((-float(err) / 100)*2.5 + ((err - perr)/(rospy.get_time() - ptime))*1/50/100)*0.02 #+ (serr*dt)*1/20/100 #1 is best, starting 3 unstable
      serr = err + serr
      perr = err
